...
class APKtoAABConverter:
    def __init__(self):
        ...
        self.aapt2 = self.tools_dir / "aapt2"           # No .exe on Linux
    ...
    def _find_java(self):
        """Find Java path for Render"""
        java_paths = [
            "/opt/render/project/src/java/jdk-17.0.12+7/bin/java",  # Render build path
            "/usr/bin/java",
            "java"
        ]
    ...
    def _run_command(self, cmd):
        """Run command with proper Java environment"""
    ...
    def decompile_apk(self, apk_path, output_dir):
        cmd = [self.java, "-jar", str(self.apktool), "d", str(apk_path), 
               "-o", str(output_dir), "-f"]
    ...
    def _restructure_zip(self, input_zip, output_zip, decompile_dir):
        ...
        with zipfile.ZipFile(output_zip, 'w', zipfile.ZIP_DEFLATED) as new_zip:
            ...
            for file_path in extract_dir.rglob('*'):
                if file_path.is_file() and file_path.name != "AndroidManifest.xml":
    ...
    def build_aab(self, base_zip, output_aab):
        cmd = [self.java, "-jar", str(self.bundletool), 
               "build-bundle", f"--modules={base_zip}", f"--output={output_aab}"]
        self._run_command(cmd)
    ...
    def convert(self, apk_path, min_sdk=21, target_sdk=33):
        ...
        try:
            print(f"🔄 Converting: {apk_path.name}")
            ...
            print(f"✅ Conversion successful: {output_aab.name}")
